...
class ImportGamesplanetJson extends Command
{
    protected $signature = 'gamesplanet:import-json';

    protected $description = 'Import Gamesplanet prices from data/gamesplanet_prices.json';
    ...
    public function handle(): int
    {
        ...
        if (!file_exists($path)) {
            $this->warn("Gamesplanet JSON not found at {$path}");
            return self::SUCCESS;
        }
        ...
        if (empty($items)) {
            $this->warn('No Gamesplanet prices found in JSON.');
            return self::SUCCESS;
        }
        ...
        foreach ($items as $item) {
            ...
            if (!$game) {
                Log::debug("Gamesplanet import: no game match for '{$cleanTitle}' (from '{$name}')");
                continue;
            }
        ...
        $this->info("Gamesplanet import: created {$created}, updated {$updated} products.");
    ...
    private function extractGameTitle(string $gamesplanetName): string
    {
        $title = preg_replace('/\s*-\s*(PC|Steam|GOG|Epic Games|Xbox|PlayStation|Nintendo).*/i', '', $gamesplanetName);
