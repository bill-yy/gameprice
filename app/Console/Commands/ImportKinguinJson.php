...
class ImportKinguinJson extends Command
{
    protected $signature = 'kinguin:import-json';

    protected $description = 'Import Kinguin prices from data/kinguin_prices.json';
    ...
    public function handle(): int
    {
        ...
        if (!file_exists($path)) {
            $this->warn("Kinguin JSON not found at {$path}");
            return self::SUCCESS;
        }
        ...
        if (empty($items)) {
            $this->warn('No Kinguin prices found in JSON.');
            return self::SUCCESS;
        }
        ...
        foreach ($items as $item) {
            ...
            if (!$game) {
                Log::debug("Kinguin import: no game match for '{$cleanTitle}' (from '{$name}')");
                continue;
            }
        ...
        $this->info("Kinguin import: created {$created}, updated {$updated} products.");
    ...
    private function extractGameTitle(string $kinguinName): string
    {
        $title = preg_replace('/\s*-\s*(PC|Steam|GOG|Epic Games|Xbox|PlayStation|Nintendo).*/i', '', $kinguinName);
